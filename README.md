...
- Enabled Anhoch and Setec price scans run immediately and independently, then at `price_check_interval`; the initial full-catalog price snapshot is silent. Anhoch full-catalog scans request 500 products per page, cap each response at 2 MiB, and allow up to 100 bounded pages (200 MiB total). Setec full-catalog scans request 100 products per page, allow up to 100 pages (10,000 products), cap each response at 2 MiB, and allow 200 MiB total. Setec products without a current first-variant price are skipped without deleting prior snapshots.
