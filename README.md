...
- SQLite delivery history and feed-scoped persistent Anhoch and Neksio selling-price snapshots
...
`price_check_interval: 3600` opts an Anhoch or Neksio feed into an immediate, silent full-catalog selling-price baseline and then hourly price checks. To enable it in a Compose deployment, add that line beneath the selected Anhoch or Neksio feed in the active `/app/config/config.yaml`; do not put it on a different strategy. Omit the key or set it to `null` to disable price monitoring. Setec supports new-product discovery only and does not support price monitoring.
...
| `price_check_interval` | Anhoch or Neksio only. Set to `3600` for hourly full-catalog selling-price checks; omit or set to `null` to disable. |
...
- Anhoch and Neksio selling-price snapshots are stored persistently in the same SQLite database, scoped by feed and product.
...
- Setec checks at most the latest 30 products and seeds the first successful fetch without notifications. Setec does not support price monitoring.
- Enabled Anhoch and Neksio price scans run immediately and then at `price_check_interval`; each initial price snapshot is silent and later changes are posted. Anhoch full-catalog scans request 500 products per page, cap each response at 2 MiB, and allow up to 100 bounded pages. Neksio price scans use the same full-category bounds as discovery, with each response capped at 1 MiB.
