...
Paginator links are evidence only and are never followed; requests are built independently from the configured URL. Each `ul.pagination a[href]` must resolve to HTTPS, effective port 443, no credentials, no fragment, the exact configured origin, and an accepted search path. Its query must preserve all configured filters plus the forced sort/layout values and contain exactly one `page` value. That value must be either a positive decimal or the same positive decimal followed by the exact literal suffix ` prev-nextPage`, which Reklama5 emits on previous-page controls; validation normalizes the accepted suffix before comparing filters and recording page evidence. Only that normalized page value may differ from the current request.
